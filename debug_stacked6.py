...
# Precompute features exactly like backtest engine does
...
for idx in range(len(df)):
    ...
    # EXACT BACKTEST PATH: precomputed features
    precomputed = precomputer.get_features_for_tick(idx)
    state = fe.update(ob, trades, detect_patterns=(idx % 25 == 0), precomputed_features=precomputed)
    ...
    for cond in strat.entry_conditions + strat.filters:
        if cond.feature not in f:
            missing_feats[cond.feature] = missing_feats.get(cond.feature, 0) + 1
...
if missing_feats:
    print(f'\nMissing features:')
    for k, v in sorted(missing_feats.items(), key=lambda x: -x[1])[:10]:
        print(f'  {k}: {v}')
else:
    print(f'\nNo missing features')
